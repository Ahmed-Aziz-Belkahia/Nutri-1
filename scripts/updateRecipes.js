...
async function updateRecipes() {
  ...
  // We need to use a direct SQL query to update the recipes
  // since we can't use cookies with node-fetch to maintain the session
  console.log('Using direct database updates for recipes...');

  const updateQueries = Object.entries(recipeUpdates).map(([recipeId, updates]) => {
    // Convert the ingredients array to a JSON string
    const ingredientsJson = JSON.stringify(updates.ingredients);
    
    // Convert the instructions array to a JSON string
    const instructionsJson = JSON.stringify(updates.instructions);
  ...
  console.log('Generated the following update queries:');
  updateQueries.forEach(query => console.log(query));
  
  console.log('\nPlease run these queries directly against your database using the execute_sql_tool');
}
